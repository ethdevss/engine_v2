...
class IndicatorGenerator(object):
    ...
    def is_5m_candle_close(cls, minute, second):
        is_just = (second >= 3 and second <= 10)
...
class BitmexAPIToolKit(object):
    bitmex_host = "https://testnet.bitmex.com"
    api_key = "0nyCXeqgPpLVZgvhqSUjYIPG"
    api_secret = "KkPcqOVy4YyOpzEykv1dmn-9ehU3ORqYAH5L2Qvd9RcsPDmC"
...
def add_strategy(request):
    if request.headers['Authorization'] == authorization_key:
        ...
        strategy = DivergenceStrategy(70, 100)
        strategy_scheduler.add_scheduler('interval', "2", strategy.run, 9)
